...
**v2.82.0 (Phase 236 COMPLETE)** — 2525 passed, 5 skipped.
...
- **WebQSP Benchmark (Phase 231)**: `benchmarks/webqsp_param_eval.py` — WebQSP KGQA benchmark (1,628 test questions, Freebase 2-hop KB, 3.79M triples). 2-pass seed-entity subgraph extraction (`load_seed_subgraph`): Pass 1 collects ≤150 edges/seed, Pass 2 expands 1-hop nodes at ≤20/node — yields 584k triples (292k nodes) vs 1.3M-node OOM on full load. MID relay-node filter: `/m/xxxxx` nodes excluded from ranked output (CVT intermediate nodes, never correct answers). `build_webqsp_state()` + `run_trial_inprocess()` in-process pattern; `PARAM_SPACE_WEBQSP` + `--dataset webqsp` tuner integration. Phase 231 zero-config baseline: **H@1=5.50%, H@10=25.50%, MRR=0.1127** (200q, random, Hetionet fallback params). Key finding: Freebase CVT reification creates redundant parallel paths that inflate vote counts for incorrect answers; CVT-aware path deduplication is the architectural fix (future work).
- **Question Decomposition + Answer-Type Filtering (Phase 232)**: `core/question_decomposer.py` — `QuestionDecomposer.decompose(question)` → `DecomposedQuestion(answer_type, relation_keywords, wh_word, has_temporal_constraint, is_comparative)`. Training-free: WH-word → answer type ("who"→person, "where"→place, "when"→time, "how many"→quantity), verb lemmatization (87 forms: "plays"→"play", "directed"→"direct"), stopword removal. `core/relation_name_index.py` — `RelationNameIndex.build_from_relations()` + `score_relations(question_tokens)`: tokenizes dotted Freebase relation names, scores via Jaccard overlap + verb synonym bonus (17 verb synonym sets). WebQSP integration: `build_webqsp_state()` indexes all 4,166 graph relations; `run_trial_inprocess()` post-extraction re-ranks via path terminal-relation matching; `_soft_type_filter()` pushes non-person-like entities down for "who" questions. Phase 232 result: **H@1=6.5%** vs 5.5% Phase 231 baseline (+18% relative). 84% question coverage. Key architectural insight: goal-directed reasoning requires answer-type representation + relation-path steering, not just signal aggregation.
- **Training-Free Path Schema Prediction (Phase 236)**: `core/path_schema_index.py` — `PathSchemaIndex`: first PREDICTIVE reasoning signal (all prior signals are reactive). Enumerates all (r1, r2) 2-hop relation schemas via intermediate-node cross-product (A→r1→B→r2→C); embeds each as last-segment Freebase text (`person.person.place_of_birth`→`"place of birth"`); stores L2-normalised float16 matrix. `predict_schemas_for_seed(qemb, seed_outgoing_rels)`: filters candidates to schemas whose r1 is an actual outgoing relation from the seed (prevents structurally inapplicable matches), ranks by cosine similarity. `execute_schemas(seed, schemas, adapter)`: targeted 2-hop traversal — hop1 via r1, hop2 via r2 from each intermediate. Top-2 schema answers prepended to beam ranked list for H@1 competition; remaining appended for coverage. MultiDiGraph fix in `adapters/networkx_adapter.py`: `get_neighbors` with `edge_types` now iterates inner edge dicts (`raw.values()`) for MultiDiGraph; unfiltered path preserves RELATED_TO fallback to maintain beam scoring consistency. 81,755 schemas indexed, 27 unit tests. Phase 236 result: **H@1=9.5%, H@10=32.5%, MRR=0.1552** vs Phase 235 H@1=6.0%, H@10=28.5%, MRR=0.1198 (**+3.5pp H@1, +4.0pp H@10, +29% MRR**).
- **Community-Structured Hypothesis Generation (Phase 233)**: `core/community_hypothesis.py` — `CommunityHypothesisGenerator.build(adapter)`: O(|E|) scan builds `_bridge_index: Dict[(src_cid, dst_cid), Counter[rel]]` and `_outbound_index: Dict[cid, Counter[rel]]` from all inter-community edges. `generate_hop_boosts(entity, top_n, boost_scale)` → boost dict for top-N community bridge relations. `generate_typed_boosts(entity, answer_type)` → filters to bridge relations matching expected answer type via Freebase relation-suffix heuristics (last dot-segment: "actor"→person, "containedby"→place, "date"→time). `_community_reach_types[cid]` = frozenset of answer types reachable from community. `reasoning/traversal.py` adds `_community_hypothesis_fn` kwargs: per-entity callable applied at non-terminal hops only (terminal hop reserved for TRB). `core/cerebrum.py` query() adds `community_hypothesis_fn` parameter. 219,588 community-pair bridges indexed on WebQSP graph. Phase 233 result (200q, ±1.7pp noise): **H@1≈7–8%, H@10≈23–25%, MRR≈0.12** vs Phase 232 baseline H@1=6.5%, H@10=25.5%, MRR=0.1127. Novel property: works on opaque relation names, purely topology-derived, language-agnostic.
...
- **2525 passed, 5 skipped** as of v2.82.0 / Phase 236
