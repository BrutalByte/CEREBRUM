...
\begin{enumerate}
  ...
  \item \textbf{PathSchemaIndex:} The first predictive reasoning signal in the
    system that predicts the most likely $(r_1, r_2)$ 2-hop relation path before
  ...
  \item \textbf{Honest WebQSP characterisation:} On WebQSP (Freebase KB with
    ...
    of the residual gap as Freebase CVT mediator nodes that deactivate the
...
\begin{itemize}
  ...
  \item \textbf{Zero-config baseline (MetaQA 3-hop):} H@1=56.8\%, H@10=90.7\%
    with ParameterInitializer defaults, with no tuning required.
