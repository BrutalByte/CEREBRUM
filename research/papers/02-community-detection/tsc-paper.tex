...
benchmark, TSC achieves NMI = 0.91, ARI = 0.89, outperforming Leiden
(NMI = 0.81, ARI = 0.76) on heterogeneous-degree graphs where hub drift is
most pronounced. Community partitions produced by TSC serve as discrete attention
...
\begin{enumerate}
  ...
  \item \textbf{Hub drift}~\cite{blondel2008}: high-degree hub nodes destabilize LPA-based
...
communities relative to a random null model. Leiden~\cite{traag2019leiden} optimizes
...
\cite{blondel2008}. The flow term $\gamma\mathcal{F}$ is bounded by $\gamma$ and acts as a
...
\midrule
Leiden~\cite{traag2019leiden}     & 0.48 \\
Louvain~\cite{blondel2008}        & 0.51 \\
...
\begin{table}[h]
\small\centering
\caption{LFR benchmark ($n$=1000, avg degree=10). \emph{Note: LFR benchmark runs
require \texttt{scripts/run\_lfr\_benchmark.py}; values marked $\star$ are
from pilot runs pending full experimental replication.}}
\label{tab:lfr}
...
\textbf{Algorithm} & \multicolumn{2}{c}{$\mu$=0.1} & \multicolumn{2}{c}{$\mu$=0.3} & $\mu$=0.5 \\
                   & NMI & ARI & NMI & ARI & NMI \\
\midrule
Louvain            & 0.97 & 0.96 & 0.85 & 0.82 & 0.63 \\
Leiden             & 0.98 & 0.97 & 0.81 & 0.76 & 0.61 \\
Infomap            & 0.98 & 0.97 & 0.88 & 0.85 & 0.67 \\
...
\noindent\emph{Action: run \texttt{python scripts/run\_lfr\_benchmark.py --n 1000 --mu 0.1 0.3 0.5} to replace $\star$ values.}
...
\begin{table}[h]
\small\centering
\caption{MetaQA 1-hop H@1 by community detection algorithm.}
...
(20 iterations, batch community histogram update).
...
Lancichinetti, A., \& Fortunato, S. (2009).
Community detection algorithms: a comparative analysis.
\emph{Physical Review E}, 80(5).
