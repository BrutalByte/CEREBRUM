...
"""
    ...
    def synthesize_goals(self, insights: List[Any]):
        """Phase 114: Synthesize goals based on insights."""
        from core.goal_system import make_goal
        # Example goal synthesis logic
        for insight in insights:
            # If the insight points to a low-coherence area, create a goal to explore it
            if "low-coherence" in getattr(insight, "description", "").lower():
                new_goal = make_goal(
                    description=f"Improve coherence for insight: {insight.description}",
                    metric_type="soliton_index",
                    target_value=0.85,
                    priority=2
                )
                new_goal.autogenerated = True
                logger.info(f"Researcher: Synthesized goal {new_goal.id} from insight.")
                return new_goal
        return None
