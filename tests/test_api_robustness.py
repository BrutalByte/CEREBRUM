...
def test_api_9_parameter_query(api_client):
    ...
    res = api_client.post("/query", json=payload)
    assert res.status_code == 200
    data = res.json()
...
def test_api_streaming_fallback(api_client):
    ...
    res = api_client.get("/stream/status")
    assert res.status_code == 200
    data = res.json()
    assert data["nodes"] == 2
...
def test_api_insight_endpoints(api_client):
    ...
    # status
    res = api_client.get("/insight/status")
    assert res.status_code == 200
    assert "total_events" in res.json()
    
    # scan (dry run or on-demand)
    res = api_client.post("/insight/scan")
    assert res.status_code == 200
...
def test_api_unauthorized(api_client):
    ...
    # Try with wrong key
    client_bad = TestClient(api_client.app, headers={"X-API-Key": "wrong"})
    res = client_bad.get("/health")
