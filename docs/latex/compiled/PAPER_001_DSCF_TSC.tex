...
\textbf{TSC as an Explicit Mode (Phase 49).} Prior to v2.24.0, DSCF and TSC were treated as a combined pipeline with TSC as a refinement pass. From v2.51.0, TSC is an explicitly selectable community detection mode (\texttt{CommunityEngine(mode="tsc")}), allowing pract\-itioners to benchmark it cleanly against DSCF and Leiden baselines. Vectorized TSC retains its Q=0.88 advantage on caveman graphs while adding confi\-gurable temperature schedules.
...
\textbf{MetaQA Canonical Benchmark Results.} With adaptive community-driven beam parameters, CEREBRUM v2.51.0 achieves the following canonical results on MetaQA:
...
\textbf{Test Coverage.} The full CEREBRUM test suite now comprises 2175+ tests (up from 994 at v2.24.0), with dedicated regression suites covering TSC stability, community swap atomicity, and modularity drift detection.
