...
[![Version](https://img.shields.io/badge/version-v2.85.0-indigo)](https://github.com/BrutalByte/CEREBRUM)
...
> **Phase 244 re-tuning in progress.** Phase 241 full-set tuner achieved **H@1=5.04%, H@10=11.86%** on all 1,628 questions (zero-config baseline: H@1=1.41%). Phase 239 fANOVA identified `degree_penalty_weight` as the dominant parameter (50% of variance). Phase 243 wired `cvt_passthrough` into the pipeline — BeamTraversal now collapses A→CVT→B into compound edges scored against named leaf entities, fixing the root cause of ~60% of H@1 failures. Phase 244 re-tunes all parameters under the corrected traversal.
