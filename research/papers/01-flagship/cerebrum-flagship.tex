...
\small
\centering
\caption{Ablation on MetaQA 3-hop H@1. Component contributions measured on 500-question sample; see \texttt{scripts/run\_ablation\_metaqa.py} for full evaluation.}
\label{tab:ablation}
\begin{tabular}{lcc}
\toprule
\textbf{Configuration} & \textbf{3-hop H@1} & \textbf{$\Delta$} \\
\midrule
Full \CEREBRUM{}$^\ddagger$  & 16.6\%   & --- \\
$-$ Bridge Twins            & 15.6\%   & $-1.0$ \\
$-$ Adaptive Beam           & 16.0\%   & $-0.6$ \\
DSCF instead of TSC         & 15.6\%   & $-1.0$ \\
$-$ GraphSAGE smoothing     & 16.6\%   &  $0.0$ \\
BFS baseline                &  1.1\%   & $-15.5$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent\emph{$^\ddagger$Full \CEREBRUM{} ablation baseline uses TSC + GraphSAGE + BridgeTwins + Bayesian beam (Phase~167). The 12.5\% in Table~\ref{tab:main} is the Phase~53 canonical training-free config without GraphSAGE or BridgeTwins. Ablation measured on 500-question sample of MetaQA 3-hop test set; run \texttt{scripts/run\_ablation\_metaqa.py} for full 14,274-question evaluation.}
