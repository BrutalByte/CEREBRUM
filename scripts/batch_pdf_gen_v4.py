...
# CEREBRUM: Professional Batch PDF Generation Engine v4 (v1.2.4)
# Harmonizes the technical library using a high-stability CLI engine.
# Features lock-retry logic and surgical metadata synchronization.
...
def convert_file(file_path):
    ...
    print(f"Syncing & Converting: {name}...")
    ...
    try:
        ...
        # 3. Rename resulting PDF to final name (with retry)
        ...
        for i in range(max_retries):
            try:
                ...
                if i == max_retries - 1:
                    print(f"   Warning: Could not overwrite {name} (Locked). Saved as temp_{name}.pdf")
                else:
                    time.sleep(1)
