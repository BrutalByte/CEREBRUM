...
# CEREBRUM: Global LaTeX Publication Engine (v1.2.4)
# Orchestrates the high-precision transformation of the entire library into LaTeX-rendered PDFs.
...
def generate_pdf(name, md_path, template_path, title, subtitle):
    ...
    def run_latex(cmd_list):
        subprocess.run(cmd_list, cwd=LATEX_BUILD_DIR, capture_output=True, check=False)

    # Clean previous aux files to avoid conflicts
    for ext in ['.aux', '.log', '.out', '.bbl', '.blg', '.pdf']:
        f_path = os.path.join(LATEX_BUILD_DIR, f"{job_name}{ext}")
        if os.path.exists(f_path): os.remove(f_path)

    run_latex(['pdflatex', '-interaction=nonstopmode', f"{job_name}.tex"])
    
    # Run BibTeX only if academic template (contains \bibliography)
    if 'academic' in template_path:
        run_latex(['bibtex', job_name])
        run_latex(['pdflatex', '-interaction=nonstopmode', f"{job_name}.tex"])
