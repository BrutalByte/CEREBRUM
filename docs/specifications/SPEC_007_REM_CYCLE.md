...
### 6. Sleep-Phase Consolidation (Phase 167)
CEREBRUM v2.51.0 introduces a unified `ConsolidationEngine` that merges traditional REM maintenance with mnemonic optimization.
...
2.  **Shortcut Synthesis (Phase 167)**: The engine identifies recurrent multi-hop trajectories in the `QueryLog`. Trajectories exceeding the `threshold` are materialized as direct `REM_SHORTCUT` edges.
