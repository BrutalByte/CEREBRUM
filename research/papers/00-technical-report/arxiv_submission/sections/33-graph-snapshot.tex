...
\section*{1. Motivation: Fragile Persistence}
...
The \texttt{metadata} field captures any arbitrary per-edge metadata written at ingest time (confidence, source, provenance batch_id, etc.).
...
\section*{4. Integration with AutonomousDiscoveryLoop}
...
Pairs with \texttt{ProvenanceLedger.rollback\_cycle()} for fine-grained recovery: if the loop materialized bad edges after the last snapshot, roll them back via provenance rather than restoring the full snapshot.

\medskip\noindent\rule{\linewidth}{0.4pt}\medskip

\section*{5. Comparison to Alternatives}
...
\midrule
\texttt{pickle} & No & No & No & No \\
CSV export & Yes & Manual & Partial & Yes \\
\textbf{GraphSnapshot} & Yes & Yes & Yes & Yes \\
